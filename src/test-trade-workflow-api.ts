...
const testCases = [
  {
    ...
    input: {
      userInput:
        '我想做股票短线交易，有什么好的策略？请提供具体的技术指标和风险控制措施。',
      maxRetries: 2,
    },
  },
  {
    ...
    input: {
      userInput:
        '我是外汇交易新手，想了解EUR/USD货币对的交易策略，包括入场时机和止损设置。',
      maxRetries: 3,
    },
  },
  {
    ...
    input: {
      userInput:
        '比特币现在适合做波段交易吗？请分析当前市场情况并给出交易建议。',
      maxRetries: 1,
    },
  },
  {
    ...
    input: {
      userInput:
        '商品期货交易中如何控制风险？请提供具体的资金管理和仓位控制策略。',
    },
  },
];
...
async function testTradeWorkflow(testCase: any, index: number): Promise<void> {
  ...
  try {
    console.log('📤 发送请求...');
    console.log(
      `   输入: ${testCase.input.userInput.substring(0, 100)}${testCase.input.userInput.length > 100 ? '...' : ''}`,
    );
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.log(
        `   错误信息: ${error instanceof Error ? error.message : error}`,
      );
    }
  }
}
